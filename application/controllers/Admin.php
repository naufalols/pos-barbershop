...
class Admin extends CI_Controller
{
    ...
    {
        $data['title'] = 'Capster Page';
        $data['capster'] = $this->db->get('tk_cukur')->result_array();
        $data['menu'] = $this->db->get('menu')->result_array();
        $date = new DateTime("now");
        $curr_date = $date->format('Y-m-d ');
        $select =   array(
        ...
        $data['nama'] = $this->session->userdata('nama');
        $this->load->view('template/adminHeader', $data);
        $this->load->view('admin/capster', $data);
        $this->load->view('admin/modals/modalCapster', $data);
        $this->load->view('template/adminFooter');
    }

    public function viewCapster()
    {
        $data = $this->input->post('id');
        if ($data) {
            $result = $this->Admin_model->capsterDetail($data);
            echo json_encode($result);
        } else {
            $result = array('error' => 1, 'result' => 'data id tidak ditemukan' );
            echo json_encode($result);
        }
    }
}
